...
@:await class DuckJet {
	...
	@:await public static function main() {
		...
		final container = new NodeContainer(5390, {upgradable: true});
    var lmtpData = (@:await AppSettings.config.duckJet.duck.lmtp).toString();
    var apiData = (@:await AppSettings.config.duckJet.jet.api).toString();
    ...
    var apiOpts:tink.json.Serialized<duck_jet.mailers.JetMailer.JetTransportOptions> = cast apiData;
		var controller = new duck_jet.Api.Impl(lmtpOpts.parse(), apiOpts.parse());
		#if duckfireEnabled
		final router = new Router<fire_duck.Session, duck_jet.Api.Impl>(controller);
		handler = req -> router.route(Context.authed(req, cast Session.new)).recover(OutgoingResponse.reportError);
		#else
		final router = new Router<duck_jet.Api.Impl>(controller);
		handler = req -> router.route(Context.ofRequest(req)).recover(OutgoingResponse.reportError);
		#end
